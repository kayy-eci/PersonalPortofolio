...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (numberEls.length)
      gsap.set(numberEls, { "--sm-num-opacity": 0 } as Record<string, unknown>);
        ...
        >
          ...
          <button
            ...
            className={`sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ${
              open ? "text-black" : "text-[#e9e9ef] cursor-target"
            }`}
          ...
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  <li
                    className="sm-panel-itemWrap relative overflow-hidden leading-none cursor-target"
                    key={it.label + idx}
                  >
                    <Link
              ...
              >
                ...
                >
                  {socialItems.map((s, i) => (
                    <li
                      key={s.label + i}
                      className="sm-socials-item cursor-target"
                    >
                      <a
                        href={s.link}
                        target="_blank"
